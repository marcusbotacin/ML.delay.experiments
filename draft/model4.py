...
#IPython.embed()
# Train
print("Training with %d samples to predict %d samples" % (dataTrain.size,dataTest.size))
model = Word2vec_RF_DDM()
model.train(dataTrain,y_train)

y_pred = model.predict(dataTrain)
print("Training [Acc: %f] [Prec: %f] [Rec: %f] [F1: %f]" % (accuracy_score(y_train,y_pred),precision_score(y_train,y_pred),recall_score(y_train,y_pred),f1_score(y_train,y_pred)))
...
#print(y_test)
    ...
    print("%d | [Acc: %f] [Prec: %f] [Rec: %f] [F1: %f]" % (epoch,accuracy_score(Y_acc, Y_pred_acc),precision_score(Y_acc, Y_pred_acc),recall_score(Y_acc, Y_pred_acc),f1_score(Y_acc, Y_pred_acc)))
