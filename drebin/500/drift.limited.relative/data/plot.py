...
files = ["nodetection.csv","best.csv","ddm.csv", "eddm.csv", "ddm3.csv", "eddm3.csv"] 

# We need to specify labels, in the correct order
# Let's try to put labels in the exposure order, for easing the reading
labels = ["50%/6:1", "50%/6:1 (DDM)", "50%/6:1 (EDDM)", "50%/6:1 (DDM/Partial)", "50%/6:1 (EDDM/Partial)"] 
...
plt.legend(loc='cneter') #ncol=2,bbox_to_anchor=(0.5, -1.05), fancybox=True)
plt.grid(axis='both')
...
fig.savefig('drift.limited.exṕosure.relative.pdf')  
